...
def check_local_repository(state: AgentState):
    """
    The Scavenger: Pulls every file from the specific Lease ID subfolder.
    """
    ...
    # 🛡️ DEFENSIVE CHECK: Ensure the path is actually a folder
    if os.path.exists(target_subfolder) and os.path.isdir(target_subfolder):
        all_files = [
            os.path.join(target_subfolder, f) 
            for f in os.listdir(target_subfolder) 
            if os.path.isfile(os.path.join(target_subfolder, f))
        ]
        
        if not all_files:
            return {
                "source_found": "EMPTY_FOLDER",
                "error_log": state["error_log"] + [f"CRITICAL: Subfolder '{l_id}' exists but is EMPTY."]
            }
    ...
    return {
        ...
        "error_log": state["error_log"] + [f"CRITICAL: Lease ID subfolder '{l_id}' missing."]
    }
