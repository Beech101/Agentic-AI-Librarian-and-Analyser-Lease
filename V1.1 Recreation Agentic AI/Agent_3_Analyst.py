...
# --- 2. MASTER AUDIT CONTROLS (Your Specific Logic) ---
# This is your Command Center. Edit the 'rule' text to tweak outputs on the fly.
AUDIT_CONTROLS = {
    ...
    "161": {"label": "Landlord_Split", "rule": "Format: 'TRUE - [CONTRACT NUMBER ME_P_0000]' or FALSE."},
...
def analyst_node(state: AgentState):
    ...
    print(f"🧠 Analyst [{l_id}]: Analyzing with Full Audit Logic...")

    # Build detailed dynamic instructions for the prompt
    instruction_block = "\n".join([f"Field {k} ({v['label']}): {v['rule']}" for k, v in AUDIT_CONTROLS.items()])

    prompt = [
        ("system", "You are an expert IFRS 16 Auditor. You are rigid, precise, and follow formatting rules exactly."),
        ("user", f"""
        TASK:
        Extract and reconcile 52 audit fields. 
        If the FACT SHEET contradicts the LEGACY DATA, prioritize the FACT SHEET (The Lease is the source of truth).
        
        SPECIFIC FIELD RULES:
        {instruction_block}

        FACT SHEET (Extracted from documents):
        {fact_sheet}

        LEGACY DATA (Existing Excel record):
        {legacy_data}
        """)
    ]
    ...
    try:
        ...
        # Map descriptive labels back to numeric keys for Agent 4
        final_results = {}
        for key, info in AUDIT_CONTROLS.items():
            val = raw_output.get(key) or raw_output.get(info['label'])
            final_results[key] = val

        return {
            "audit_results": final_results,
            "error_log": state["error_log"] + ["Agent 3: 52-field audit analysis complete."]
        }
